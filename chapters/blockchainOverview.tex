...
\tocless{\subsubsection{Darbo įrodymo strategija}
...
\enquote{Bitcoin} blokų grandinėje laukiama apytiksliai 6 blokų iš eilės, kad \textit{bloką-našlaitį} būtų galima atmesti \cite{Zheng2017}.
...
\tocless{\subsubsection{Turto įrodymo strategija}
...
\tocless{\subsubsection{Autoriteto įrodymo statregija}
...
Blokų grandinės leidžia programų, parašytų su išmaniaisiais kontraktais veikimą,
...
Egzistuoja kelios platformos, pritaikytos išmaniųjų kontraktų kūrimui. Bene žymiausia iš jų - \enquote{Ethereum}.
...
(angl. \textit{turing complete}) programavimo kalba \cite{EthereumWhitePaper}. \enquote{Ethereum} kontraktai vykdomi visų tinklo
dalyvių, \enquote{Ethereum} virtualiosios mašinos pagalba. Dalyviai už kontraktų kodo vykdymą apdovanojami šios platformos kriptovaliuta - eteriu.\\
\enquote{Ethereum} apibrėžia dviejų tipų paskyras, skirtas tinklo dalyviams atlikti transakcijas. Išorinės paskyros
...
\enquote{Ethereum} išmanieji kontraktai gali saugoti bei keisti būseną (kontrakto apibrėžtus duomenis) ar kviesti kitų kontraktų funkcijas siunčiant jiems žinutes.
Kontrakto vykdymas yra apmokestinamas \enquote{Ethereum} kuru. Kontrakte apibrėžtos funkcijos kvietėjas už kiekvieną
vykdytą operaciją, priklausomai nuo jos skaičiavimo sudėtingumo, turi susimokėti tam tikrą kuro kiekį. Kuro sistema padeda apsisaugoti nuo begalinių ciklų vykdymų - jei vykdant
...
\tocless{\subsubsection{Daugumos ataka}}
...
\tocless{\subsubsection{Plečiamumas}}
...
blokų grandinės dažnai keičia ją į kitą konsensuso strategiją - \enquote{Ethereum} blokų grandinė ketina pereiti prie turto įrodymu grįsto konsensuso \cite{Ethereum}.
...
\tocless{\subsubsection{Anonimiškumas}}
